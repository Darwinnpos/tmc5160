...
int (*SPIReadWrite)(int chl,int data, int lastchar);
...
void SPIReadWrite(uint8_t chl,uint8_t *send_data,uint8_t *recv_data)
{
    ...
    case 1:
        ...
        break;
    case 2:
        spi_dev = (struct rt_spi_device *)rt_device_find("spi21");
        break;
    case 3:
        spi_dev = (struct rt_spi_device *)rt_device_find("spi22");
        break;
    case 4:
        spi_dev = (struct rt_spi_device *)rt_device_find("spi30");
        break;
    case 5:
        spi_dev = (struct rt_spi_device *)rt_device_find("spi31");
        break;
    default:
        break;
    }
...
/*
 * An SPI read/write pointer can be defined to directly
 * match the address of the hardware dependent function,
 * completing SPI sending
 */
...
static int32_t readInt_spi(uint8_t motor, uint8_t address)
{
    ...
    data[4] = 0;
    SPIReadWrite(chl,data,NULL);
    SPIReadWrite(chl,data,recv_data);
    value |= recv_data[1] << 24;
    value |= recv_data[2] << 16;
    value |= recv_data[3] << 8;
    value |= recv_data[4] ;
    return value;
}
...
{
    switch (start_para.rampmode) {
        case TMC5160_MODE_POSITION:
            return moveTo(motor,start_para.x_target);
            break;
        case TMC5160_MODE_VELPOS:
        case TMC5160_MODE_VELNEG:
            return rotate(motor,start_para.vmax);
            break;
        default:
            break;
    }
...
uint32_t handleParameter(uint8_t readWrite, uint8_t motor, uint8_t type, int32_t *value)
{
    uint32_t buffer;
...
uint32_t userFunction(uint8_t type, uint8_t motor, int32_t *value)
{
    ...
    /*
     * The hardware engineers have completed the configuration.
     * Therefore, you can choose this section according to your
     * needs.
     * Author:Fpi_lc Data:2021.11.27
     */
...
void init_comm(TMC_Board_Comm_Mode mode)
{
...
uint32_t TMC5160_init(uint8_t motor)
{
    ...
    /*Initialize the hardware peripherals. If you have already
     * initialized them, you do not need to do this step.
     * Author: Fpi-lc
     */
    init_comm(TMC_COMM_SPI);

    //
    spi_mutex = rt_mutex_create("SPI_MUX", RT_IPC_FLAG_PRIO);
    /*
